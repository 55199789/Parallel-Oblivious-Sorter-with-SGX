...
static sgx_status_t SGX_CDECL sgx_ecall_DPSort(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_DPSort_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_DPSort_t* ms = SGX_CAST(ms_ecall_DPSort_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	int* _tmp_cArr = ms->ms_cArr;


	ecall_DPSort(_tmp_cArr, ms->ms_cnt, ms->ms_epsilon);
...
static sgx_status_t SGX_CDECL sgx_ecall_OneByOne(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_OneByOne_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_OneByOne_t* ms = SGX_CAST(ms_ecall_OneByOne_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	int* _tmp_pArr = ms->ms_pArr;
	int* _tmp_cArr = ms->ms_cArr;


	ecall_OneByOne(_tmp_pArr, _tmp_cArr, ms->ms_cnt);
...
static sgx_status_t SGX_CDECL sgx_ecallEncryptArr(void* pms)
{
	...
	_in_retval = ecallEncryptArr(_tmp_pSrc, _tmp_pDst, ms->ms_cipher_len);
...
static sgx_status_t SGX_CDECL sgx_ecallDecryptArr(void* pms)
{
	...
	_in_retval = ecallDecryptArr(_tmp_pSrc, _tmp_pDst, ms->ms_cipher_len);
	if (MEMCPY_S(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
...
SGX_EXTERNC const struct {
	...
	7,
	{
		{(void*)(uintptr_t)sgx_ecall_DPSort, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_ObliviousSort, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_OneByOne, 0, 0},
